...
public class infoMyAccountFragment extends Fragment {
    ...
    public View onCreateView(
        ...
        if (getActivity().getIntent().getExtras().getString("id", "").equals("")) {
            id = getToken(getActivity());
        } else {
            id = getActivity().getIntent().getExtras().getString("id");
        }
        showUserProfileViewModel.getUserProfile(getActivity(), id).observe(getActivity(),
        new Observer<User>() {
            @Override
            public void onChanged(User user) {
                ...
                myProductViewModel.getMyProducts(getActivity(), user.getId()).observe(getViewLifecycleOwner(), new Observer<ArrayList<Product>>() {
                    ...
                    public void onChanged(ArrayList<Product> myProducts) {
                        recycleAdapterProduct = new RecycleAdapterProduct(getActivity(), myProducts, "my", showUserProfileViewModel);
